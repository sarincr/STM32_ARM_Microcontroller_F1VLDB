...
**/
	GPIO_InitTypeDef GPIOXDEF;
void BlinkInit()
{
					/*Define a GPIO configuration variable*/
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC , ENABLE); 	/*clock of GPIOC enable*/
	GPIOXDEF.GPIO_Pin = GPIO_Pin_8|GPIO_Pin_9;				/* pin 5*/
	GPIOXDEF.GPIO_Mode = GPIO_Mode_Out_PP;			/*push-pull output mode*/
	GPIOXDEF.GPIO_Speed= GPIO_Speed_50MHz;			/*GOIO Output frequency*/
	GPIO_Init(GPIOC, &GPIOXDEF);
}

void Delay(uint32_t times){
	for(;times>0;times--);
}
TIM_TimeBaseInitTypeDef TIMDEFe;
...
**/
	...
	TIMDEFe.TIM_Period =period;  
	TIMDEFe.TIM_Prescaler =prescale; 	
	TIMDEFe.TIM_ClockDivision = TIM_CKD_DIV1;	/* source clock without division*/
	TIMDEFe.TIM_CounterMode = TIM_CounterMode_Up;/*count up*/	
	TIMDEFe.TIM_RepetitionCounter = 0; /*update every time when counter overflows*/
	TIM_TimeBaseInit(TIM2,&TIMDEFe);	/*initialize interrupt*/
...
int main(void){
	...
	TIM2_Configuration();
while(1){
		GPIO_SetBits(GPIOC,GPIO_Pin_9);	//set PB5 high, led off
		Delay(7000000);
		GPIO_ResetBits(GPIOC,GPIO_Pin_9);//set PB5 low, led on
		Delay(7000000);	
	}
}
